	...
	<head>
		...
		<script>
			$(function(){
				//基本变量初始
				setGlobaVariable({
					inputURL:'<s:url namespace="/plugin/article" action="input?workingBodyId=' + $('#workingBodyId').val() + '&channelId=' + $('#channelId').val() + '"/>',
					queryURL:'<s:url namespace="/plugin/article" action="query?workingBodyId=' + $('#workingBodyId').val() + '&channelId=' + $('#channelId').val() + '"/>',
					deleteURL:'<s:url namespace="/plugin/article" action="delete?workingBodyId=' + $('#workingBodyId').val() + '&channelId=' + $('#channelId').val() + '"/>',
					editwidth:1000,
					editheight:700
				});
				//数据表格定义 						
			...
			function articleReload(){
				var url='<s:url namespace="/plugin/article" action="query"/>';
			...
			function initOperateQuery(){
				var url='<s:url namespace="/plugin/article" action="query"><s:param name="channelId" value="channelId"></s:param></s:url>';
			...
			function addOperate(){
				...
				window.open('<s:url namespace="/plugin/article" action="input?' + url_param + '"/>','popup','width=1200,height=700,resizable=yes,toolbar=no,directories=no,location=no,menubar=no,status=no,left=0,top=0'); 
				return false;
			}
			function updOperate(){
			...
			function moveOperate(){
				$('#tt2').tree({
					checkbox: false,
					url: '<s:url namespace="/plugin/onlineoffice/workingbody" action="tree"/>?channelId=' + $('#channelId').val() + '&isMatter=false'
				});
	            var rows = $("#tt").datagrid("getSelections");
	            if(rows.length == 0){
			...
			function moveArticle(){
				...
				var url = "<s:url action='move' namespace='/plugin/article'/>";
