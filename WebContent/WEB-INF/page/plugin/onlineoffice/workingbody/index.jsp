...
<html>
	<head>
		...
		<script>
			...
			$(function(){
				...
				$('#tt2').tree({
					checkbox: false,
					url: '<s:url namespace="/plugin/onlineoffice/workingbody" action="tree"/>?channelId=' + $('#channelId').val() + '&isMatter=true',
					onClick:function(node){
						workingBodyId = node.id
						var url='<s:url namespace="/plugin/onlineoffice/workingbody" action="queryChannelStatus"/>';
				...
				$('#cc_matter').combobox({
				    url:'<s:url namespace="/plugin/onlineoffice/workingbody" action="findAllMatter"/>',
				    valueField:'id',
				    textField:'name'
				});
				$('#cc_citizen').combobox({
				    url:'<s:url namespace="/plugin/onlineoffice/workingbody" action="findAllCitizen"/>',
				    valueField:'id',
				    textField:'name'
				});
				$('#systemtab').tabs({
					onSelect:function(title){
						if (title == '信息编辑'){
							...
							$('#edit_matter').attr('src','<s:url namespace="/plugin/onlineoffice/matter" action="index"/>');
						}
					}
				});
			});
			...
			function addWorkingBody(){
    			...
    			if (type == 'component' || type == 'workingbody'){
					$.messager.prompt(node.text, '请输入专栏名', function(r){
						if (r){
				            $.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="addWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'parentId':node.id,'workingBodyName':r},function(data){
					            if(data == 'matter'){
			...
			function renameWorkingBody(){
    			...
    			if (type == 'workingbody'){ 		
					...
					$.messager.prompt(node.text, '请修改专栏名', function(r){
						if (r){
				            $.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="renameWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'workingBodyId':node.id,'workingBodyName':r},function(data){
					            if(data == 'false'){
			...
			function delWorkingBody(){
    			...
    			if (type == 'workingbody'){ 		
	    			$.messager.confirm('提示', '确认要删除 ' + node.text + ' 专栏?', function(r){
	    				if (r){
	    	    	    	//删除专栏 			
	    		            $.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="delWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'workingBodyId':node.id},function(data){
	    			            if(data == 'false'){
			...
			function addMatter(){
				...
				$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="addMatterToWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'selectIds':val.toString(),'parentId':workingBodyId},function(data){
					 if(data == 'false'){
			...
			function removeMatter(){
    			...
    			if (type == 'matter'){
        			...
        			if (type == 'workingbody'){
        				$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="removeMatterFromWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'matterId':node.id,'parentId':node_parent.id},function(data){
            				if (data == 'true'){
			...
			function addCitizen(){
				...
				$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="addCitizenToMatter"/>',{'selectIds':val.toString(),'matterId':workingBodyId},function(data){
					 if(data == 'false'){
			...
			function removeCitizen(){
				...
				var type = node.attributes.type;
    			if (type == 'matter'){
        			$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="removeCitizenFromMatter"/>',{'matterId':node.id,'matterId':node.id},function(data){
            			if (data == 'false'){
			...
			function pubLeadingWindow(){
				$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="pubLeadingWindow"/>', {'channelId':$('#channelId').val()}, function(data){
					if (data == 'true'){
			...
			function upWorkingBody(){
    			...
    			if (type == 'workingbody'){ 
	    			$.messager.confirm('提示', '确认要上移 ' + node.text + ' 专栏?', function(r){
	    				if (r){
	    					var node_parent = $('#tt2').tree('getParent',node.target);
	    		            $.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="upWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'parentId':node_parent.id,'workingBodyId':node.id},function(data){
	    			            if(data == 'false'){
			...
			function downWorkingBody(){
    			...
    			if (type == 'workingbody'){ 		
	    			$.messager.confirm('提示', '确认要下移 ' + node.text + ' 专栏?', function(r){
	    				if (r){
	    					var node_parent = $('#tt2').tree('getParent',node.target);
	    		            $.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="downWorkingBody"><s:param name="channelId" value="channelId"></s:param></s:url>',{'parentId':node_parent.id,'workingBodyId':node.id},function(data){
	    			            if(data == 'false'){
			...
			function addOrgan(){
    			...
    			if (type == 'workingbody'){
    				...
    				$('#tt3').tree({
    					checkbox: true,
    					cascadeCheck:false,
    					url: '<s:url namespace="/business/organ" action="tree"></s:url>'
    				});
			...
			function saveOrgan(){
				if (selMode == 'workingbody'){
					...
					$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="addOrganToWorkingBody"/>',parameter,function(data){
						if(data == 'false'){
					...
					$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="addOrganToMatter"/>',{'matterId':tt2_node_id,'organId':node.id},function(data){
						if(data == 'false'){
			...
			function removeOrgan(){
    			...
    			var type = node.attributes.type;
    			if (type == 'workingbody'){
					$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="removeOrganFromWorkingBody"/>',{'workingBodyId':node.id},function(data){
						if (data == 'false'){
					...
					$.post('<s:url namespace="/plugin/onlineoffice/workingbody" action="removeOrganFromMatter"/>',{'matterId':node.id},function(data){
						if (data == 'false'){
							$.messager.alert('提示','移除组织失败','info');
							return;
						}
	...
	<body>
		<div class="easyui-tabs" id="systemtab" border="false" fit="true">
			<div title="信息编辑" style="padding: 0px;" icon="icon-workingbody">
				<div class="easyui-layout" fit="true">
					<div region="west"  title='<img src="<s:url value="/source/theme/icons/reload.png"/>" style="vertical-align: middle;cursor:pointer;" onclick="channelTreeLoad();"/> 网上办事<font color="red">[组织机构]</font><font color="blue">[公民信息]</font>' split="true" style="width:247px;">
						<ul id="tt2"></ul>
					</div>
			...
			<div title="事项基本信息" style="padding: 0px;" icon="icon-matter">
				<iframe src="<s:url namespace='/plugin/onlineoffice/matter' action='index'/>" id="edit_matter"  name="edit_matter" class="editifr" scrolling="no"></iframe>
			</div>
		</div>	
