...
<html>
		...
		<script type="text/javascript">
			...
			$(function() {
				...
				$('#tt').datagrid({
					...
					nowrap : true,
					striped : true,
					url : '<s:url namespace="/plugin/visit" action="cityTable"/>?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val() + '&country=' + $('#country').val() + '&province=' + $('#province').val(),
				    columns:[[  
				            {field:'city',title:'名称',width:120},
				            ...
				            {field:'trend',title:'时间趋势',width:70,
				            	formatter : function(val, rec){	
				            		return '<a href="javascript:void(0)" style="text-decoration: none" onclick="openTrend(\'' + rec.city + '\')">时间趋势</a>';
				            	}
				            }
				    ]]  
				});
			...
			function showChart(){
				...
				$.post('<s:url namespace="/plugin/visit" action="cityReport"/>', parameter, function(result) {
			...
			function openTrend(value){
				...
				var url = '<s:url namespace="/plugin/visit" action="cityTrend"/>?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val() + '&country=' + $('#country').val() + '&province=' + $('#province').val() + '&city=' + value;
				ewcmsBOBJ.openWindow("#pop-window",{url:url,width:660,height:330,title: $('#country').val() + " >> " + $('#province').val() + " >> " + value +  " 时间趋势"});
			}
		</script>
	...
	<body class="easyui-layout">
		 ...
		 <s:hidden name="province" id="province"/>
		 <div region="north" style="height:310px">
			<table width="100%" border="0" cellspacing="6" cellpadding="0"style="border-collapse: separate; border-spacing: 6px;">
				<tr>
					<td>
						当前报表：<a style="text-decoration: none" href="<s:url namespace='/plugin/visit' action='country'/>?startDate=<s:property value='%{startDate}'/>&endDate=<s:property value='%{endDate}'/>">全部</a> >> <a style="text-decoration: none" href="<s:url namespace='/plugin/visit' action='province'/>?country=<s:property value='%{country}'/>&startDate=<s:property value='%{startDate}'/>&endDate=<s:property value='%{endDate}'/>"><s:property value='%{country}'/></a> >> <font color="red"><s:property value='%{province}'/></font> 区域分布
					</td>
				</tr>
				<tr>
					<td>
