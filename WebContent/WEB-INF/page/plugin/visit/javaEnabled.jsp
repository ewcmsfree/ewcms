		...
		<script type="text/javascript">
			...
			$(function() {
				...
				$('#tt').datagrid({
					...
					nowrap : true,
					striped : true,
					url : '<s:url namespace="/plugin/visit" action="javaEnabledTable"/>?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val(),
				    columns:[[  
				            ...
				            {field:'trend',title:'时间趋势',width:70,
				            	formatter : function(val, rec){
				            		var value = false;
				            		if (rec.name=='支持Applet') value = true;
				            		return '<a href="javascript:void(0)" onclick="openTrend(\'' + rec.name + '\',\'' + value + '\')">时间趋势</a>';var value = false;
				            	}
				            }
				    ]]  
				});
			...
			function showChart(){
				...
				$.post('<s:url namespace="/plugin/visit" action="javaEnabledReport"/>', parameter, function(result) {
			...
			function openTrend(name, value){
				...
				var url = '<s:url namespace="/plugin/visit" action="javaEnabledTrend"/>?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val() + '&enabled=' + value;
	...
	<body class="easyui-layout">
		 <div region="north" style="height:310px">
			<table width="100%" border="0" cellspacing="6" cellpadding="0"style="border-collapse: separate; border-spacing: 6px;">
				<tr>
					<td>
						当前报表：浏览器&nbsp;&nbsp;&nbsp;&nbsp;从 <ewcms:datepicker id="startDate" name="startDate" option="inputsimple" format="yyyy-MM-dd"/> 至 <ewcms:datepicker id="endDate" name="endDate" option="inputsimple" format="yyyy-MM-dd"/> <a class="easyui-linkbutton" href="javascript:void(0)" onclick="view();return false;">查看</a> <a class="easyui-linkbutton" href="javascript:void(0)" onclick="view();return false;">刷新</a>
					</td>
				</tr>
				<tr valign="top">
					<td>
