		...
		<script type="text/javascript">
			...
			$(function() {
				...
				$('#tt').datagrid({
				    ...
				    columns:[[  
				            {field:'country',title:'名称',width:120,
				            	formatter : function(val, rec){
				            		return '<a style="text-decoration: none" href="javascript:void(0);" onclick="openProvince(\'' + val + '\')">' + val + '</a>';
				            	}
				            },
				            ...
				            {field:'trend',title:'时间趋势',width:70,
				            	formatter : function(val, rec){	
				            		return '<a style="text-decoration: none" href="javascript:void(0);" onclick="openTrend(\'' + rec.country + '\')">时间趋势</a>';
				            	}
				            }
				    ]]  
				});
			...
			function openTrend(value){
				...
				var url = '<s:url namespace="/plugin/visit" action="countryTrend"/>?startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val() + '&country=' + value;
				ewcmsBOBJ.openWindow("#pop-window",{url:url,width:660,height:330,title: value + " 时间趋势"});
			}
			function openProvince(name){
				window.location = '<s:url namespace="/plugin/visit" action="province"/>?country=' + name + '&startDate=' + $('#startDate').val() + '&endDate=' + $('#endDate').val(); 
			}
		</script>
	...
	<body class="easyui-layout">
		 <div region="north" style="height:310px" border="false">
			<table width="100%" border="0" cellspacing="6" cellpadding="0"style="border-collapse: separate; border-spacing: 6px;">
				<tr>
					<td>
						当前报表：<font color="red">全部</font> 区域分布
					</td>
				</tr>
				<tr>
					<td>
