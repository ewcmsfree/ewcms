 ...
 * @author 吴智俊
 */
@Controller("plugin.leadingwindow.article.query")
public class QueryAction extends QueryBaseAction {
	...
	protected PageQueryable constructNewQuery(Order order) {
		String hql = "Select r From LeaderChannel As c Right Join c.articleRmcs AS r RIGHT JOIN r.article AS a Where r.deleteFlag=false And r.id In (:id) And c.id=:leaderChannelId And c.channelId=:channelId Order By a.topFlag Desc, r.published Desc Nulls Last, r.modified Desc Nulls Last, r.id";
		String countHql = "Select count(r.id) From LeaderChannel As c Right Join c.articleRmcs AS r RIGHT JOIN r.article a Where r.deleteFlag=false And r.id In (:id) And c.id=:leaderChannelId And c.channelId=:channelId";
		...
		query.setParameter("leaderChannelId", getLeaderChannelId());
	...
	protected PageQueryable constructQuery(Order order) {
		String hql = "Select r From LeaderChannel As c Right Join c.articleRmcs AS r RIGHT JOIN r.article AS a Where r.deleteFlag=false And c.id=:leaderChannelId And c.channelId=:channelId ";
		String countHql = "Select count(r.id) From LeaderChannel As c Right Join c.articleRmcs AS r RIGHT JOIN r.article AS a Where r.deleteFlag=false And c.id=:leaderChannelId And c.channelId=:channelId ";
