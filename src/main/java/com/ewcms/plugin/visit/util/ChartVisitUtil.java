...
public class ChartVisitUtil {
	public static String getLine2DChart(List<String> categories, Map<String, Map<String, Long>> dataSet, int labelCount) {
		...
		int space = new Double(Math.ceil(categories.size() * 0.95D / labelCount)).intValue();
		int count = 0;
		for (int i = 0; i < categories.size(); i++){
			String key = categories.get(i);
			if (i % space == 0)	count++;
			xml.append("<category name='" + key + "' " + ((i % space == 0) && (count <= labelCount) ? "" : "showName='0'") + "/>");
		}
		xml.append("</categories>");
	...
	public static String getPie3DChart(Map<String, Long> dataSet){
		StringBuffer xml = new StringBuffer();
	     String[] Colors = 
	       { "FF0000", "006F00", "CCCC00", "0D8ECF", "04D215", "B0DE09", "F8FF01", "FF9E01", "FF6600", "814EE6", "F234B0", 
	       "FF9966", "0099FF", "993300", "333300", "003300", "003366", "000080", "333399", "333333", "800000", 
	       "FF6600", "808000", "808080", "008080", "0000FF", "666699", "808080", "FF9900", "99CC00", "339966", 
	       "33CCCC", "3366FF", "800080", "999999", "FF00FF", "FFCC00", "FFFF00", "00FF00", "00FFFF", "00CCFF", 
	       "993366", "C0C0C0", "FF99CC", "FFCC99", "FFFF99", "CCFFCC", "CCFFFF", "99CCFF", "CC99FF", "FFFFFF", 
	       "1D8BD1", "F1683C", "2AD62A" };
	     ...
	     while (it.hasNext()){
	    	 ...
	    	 xml.append("<set value=\"" + e.getValue() + "\" name=\"" + e.getKey() + "\" color=\"" + Colors[(i % Colors.length)] +  "\"/>");
	    	 i++;
	     }
	     xml.append("</graph>");
	     return xml.toString();
