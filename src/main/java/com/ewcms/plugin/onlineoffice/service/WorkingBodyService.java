...
import com.ewcms.plugin.onlineoffice.dao.WorkingBodyDAO;
...
public class WorkingBodyService implements WorkingBodyServiceable {
	...
	public Integer addWorkingBody(Integer parentId, String name, Integer channelId) {
		...
		Integer sort = workingBodyDAO.findWorkingBodyMaxSortByWorkingBodyId(parentId, channelId) + 1;
	...
	public void downWorkingBody(Integer parentId, Integer workingBodyId, Integer channelId) {
		...
		WorkingBody workingBody = workingBodyDAO.findWorkingBodyByWorkingBodyIdAndChannelId(workingBodyId, channelId);
		if (workingBody == null) return;
		Integer sort = workingBody.getSort();
		Integer maxSort = workingBodyDAO.findWorkingBodyMaxSort(channelId);
    ...
    public Integer addArticleRmcToWorkingBody(Article article, Integer workingBodyId, Integer channelId, Date published){
		...
		WorkingBody workingBody = workingBodyDAO.findWorkingBodyByWorkingBodyIdAndChannelId(workingBodyId, channelId);
		if (workingBody == null) return null;
		List<ArticleRmc> articleRmcs = workingBody.getArticleRmcs();
		...
		workingBody.setArticleRmcs(articleRmcs);
...
//		return articleRmc.getId();
    ...
    public void delArticleRmcToWorkingBody(Integer workingBodyId, Integer articleRmcId, Integer channelId){
		...
		workingBody.setArticleRmcs(articleRmcs);
		workingBodyDAO.merge(workingBody);
		articleRmcDAO.removeByPK(articleRmcId);
    }
	...
	@Override
	public Boolean moveArticleRmcToWorkingBody(List<Integer> articleRmcIds, Integer workingBodyId, Integer moveWorkingBodyId){
		WorkingBody workingBody = workingBodyDAO.get(workingBodyId);
		...
		List<ArticleRmc> moveArticleRmcs = moveWorkingBody.getArticleRmcs();
		if (moveArticleRmcs == null || moveArticleRmcs.isEmpty()) moveArticleRmcs = new ArrayList<ArticleRmc>();
		...
		workingBody.setArticleRmcs(articleRmcs);
		moveWorkingBody.setArticleRmcs(moveArticleRmcs);
		workingBodyDAO.merge(workingBody);
